...
export default function Navbar() {
    ...
    return (
        <>
            ...
            >
                <Link to="/"><img className="h-8.5 w-auto" src="/logo.svg" alt="logo"  /></Link>
               ...
               <div className="flex items-center gap-2">
                {isLoggedIn?(
                    <div className="relative group">
                        ...
                        <div className="absolute hidden group group-hover:block top-6 right-0 pt-4 ">
                            <button onClick={() => logout()} className="bg-white/20 hover:bg-white/30 border-2 border-white/10 px-5 py-1.5 rounded">
                            Logout</button>
                        </div>
                    </div>
            ...
            <div className={`fixed inset-0 z-100 bg-black/40 backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-400 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
                <Link onClick={() => setIsOpen(false)} to = "/" className="hover:text-pink-300 transition">Home</Link>
                    ...
                    {
                        isLoggedIn ? (
                            <button onClick={() => {setIsOpen(false) ;logout()}} className="">Logout</button>
                        ) : (
